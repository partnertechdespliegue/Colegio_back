...
public interface HorarioSalonRepo extends JpaRepository<HorarioSalon, Integer>{
	...
	@Query(value="SELECT hs.* FROM horario_salon hs\r\n" + 
			"WHERE  hs.id_salon = :idSalon AND hs.id_dia_laboral = :idDiaLaboral AND (hs.hora_inicio >= :horaInicio OR hs.hora_fin >= :horaInicio) \r\n" + 
			"AND (hs.hora_inicio <= :horaFin OR hs.hora_fin <= :horaFin)", nativeQuery = true)
	public List<HorarioSalon> encontrarPorHoraInicioYFin(@Param("idSalon") Integer idSalon, @Param("idDiaLaboral") Integer idDiaLaboral, 
